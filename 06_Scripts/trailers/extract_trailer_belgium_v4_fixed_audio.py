#!/usr/bin/env python3
"""
extract_trailer_belgium_v4_fixed_audio.py
-----------------------------------------
V4 — same as V3 (no white flashes) but with the cold-open shifted earlier.
The first text beat now hits at 0.3s instead of 2.0s, holding through
5.5s so the transition into "ONE PIANO." stays smooth.
...
Output:
  03_Cuts/Landscape/Moonlight Sonata TRAILER_BELGIUM_V4_FixedAudio.mp4
"""
...
OUTPUT      = "../../03_Cuts/Landscape/Moonlight Sonata TRAILER_BELGIUM_V4_FixedAudio.mp4"
START_TIME  = 230.0
DURATION    = 45.0
...
# Cold open hits IMMEDIATELY (0.3s) instead of 2.0s, holds to 5.5s.
filters.append(dt("A MOONLIGHT", BEBAS, 120, "(h-text_h)/2 - 40", 0.3, 5.5, attack=0.08, decay=0.4, borderw=3))
...
filters.append("fade=t=in:st=0:d=0.5")   # quicker fade-in so text isn't lost in black
...
print("\n🎬  GENERATING BELGIUM TRAILER V4 (FIXED AUDIO, EARLY OPENER) — 45s landscape...")
