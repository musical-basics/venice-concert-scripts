#!/usr/bin/env python3
"""
extract_trailer_belgium_v2.py
-----------------------------
Cinematic 45s Belgium concert trailer — V2 upgrade.

Improvements over V1:
  • "Snap-in" text with very fast attack + slow decay for trailer punch
  • White flash hits between text beats (drawbox with alpha pulse)
  • Teal + gold accent palette for key text
  • Dual-line stacked title card with spacing animation
  • Venue block with horizontal rule accent (drawn via drawbox)
  • Stronger vignette + cooler grade
  • Fade-from-black open with 0.8s audio ramp

Venue:  THEATER MALPERTUIS
...
OUTPUT = "../../03_Cuts/Landscape/Moonlight Sonata TRAILER_BELGIUM_V2.mp4"
START_TIME = 230.0
DURATION = 45.0
...
print("\n🎬  GENERATING BELGIUM TRAILER V2 (45s cinematic landscape)...")
print(f"    Source:  {INPUT} @ {START_TIME}s → {START_TIME + DURATION}s")
print(f"    Output:  {OUTPUT}\n")
subprocess.run(cmd, cwd=os.path.dirname(os.path.abspath(__file__)))
print(f"\n✅  TRAILER V2 COMPLETE! → {OUTPUT}")
