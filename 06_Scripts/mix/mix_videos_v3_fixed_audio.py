#!/usr/bin/env python3
"""
mix_videos_v3_fixed_audio.py
----------------------------
Same V3 pipeline as mix_videos_v3.py, run against the "FIXED" .mov sources
to produce an audio-only WAV master.

Differences vs mix_videos_v3.py (kept minimal):
  • KEYBOARD / WIDE point to the FIXED source files
  • OUTPUT is a .wav (PCM 16-bit), audio-only
  • ffmpeg command maps only [outa] and uses pcm_s16le

Everything else (librosa analysis, cut schedule, filter graph build,
overlays, fade timings) is identical to mix_videos_v3.py. The video
chain still gets generated but is not encoded to the output.
"""
...
KEYBOARD = "../../01_Source_Footage/Moonlight Sonata Keyboard Shot Fixed.mov"
WIDE     = "../../01_Source_Footage/Moonlight Sonata Wide Shot Fixed.mov"
OUTPUT   = "../../02_Master_Mix/Moonlight Sonata MIXED_FixedAudio.wav"
...
filter_script = "/tmp/mix_filter_v3_fixed_audio.txt"
with open(filter_script, "w") as f: f.write(";\n".join(parts))

# ── ffmpeg cmd: WAV audio-only output ──────────────────────────────────
# [outv] is routed to a null muxer (required by ffmpeg's filtergraph
# fully-consumed rule); only [outa] is encoded — to PCM WAV.
cmd = [
    "ffmpeg", "-y", "-i", KEYBOARD, "-i", WIDE,
    "-filter_complex_script", filter_script,
    "-map", "[outv]", "-f", "null", "-",
    "-map", "[outa]", "-c:a", "pcm_s16le", "-t", "350", OUTPUT,
]

print("\n🚀 RE-GENERATING V3 FIXED-AUDIO MASTER (WAV)...")
subprocess.run(cmd, cwd=os.path.dirname(os.path.abspath(__file__)))
print(f"\n✅ V3 FIXED-AUDIO COMPLETE! Output: {OUTPUT}")
