#!/usr/bin/env python3
"""
mix_canon_in_dream_v4.py
------------------------
Canon in Dream — landscape master mix, V4.

Same pipeline as mix_canon_in_dream_v3.py with one tuning change:
  • Bass-hit shake amplitude halved (5px → 2.5px) per user feedback —
    v3 felt too jittery.

Re-run target after the Wide Shot source was replaced (audio re-fixed).

Pipeline features (unchanged from v3):
  - Multi-Cam PiP (Keyboard overlay on Wide)
  - Dynamic Bass Shake (kick detection, now 2.5px amplitude)
...
BEATS_PER_CUT = 16
PIP_W, PIP_H  = 480, 270
SHAKE_AMPLITUDE = 2.5   # was 5 in v3 — halved for less jitter on bass hits
...
# Bass shake — HALVED amplitude vs v3
hits = [f"between(t\\,{o:.3f}\\,{o+0.1:.3f})" for o in bass_onsets if o < max_dur]
if len(hits) > 100:
    hits = hits[:100]
shake_expr = "0"
if len(hits) > 0:
    shake_expr = f"{SHAKE_AMPLITUDE}*(" + "+".join(hits) + ")"
...
# Visualizer, progress bar, titles, fade out
...
filter_script = "/tmp/mix_filter_canon_v4.txt"
...
print("\n🚀  RE-GENERATING CANON IN DREAM V4 MIX (shake amplitude halved)...")
print(f"    Source duration: {max_dur:.2f}s | Fade out: {fade_start:.2f}s | Shake: {SHAKE_AMPLITUDE}px")
subprocess.run(cmd, cwd=os.path.dirname(os.path.abspath(__file__)))
print(f"\n✅  CANON IN DREAM V4 COMPLETE! Output: {OUTPUT}")
